...
//返回拷贝数据后的终点位置
#define ngx_cpymem(dst,src,n)   (((unsigned char *)memcpy(dst,src,n)) + (n))
...
#define NGX_LOG_OFF               0    //控制台错误【stderr】：最高级别日志，日志的内容不再写入log参数指定的文件，而是会直接将日志输出到标准错误设备比如控制台屏幕
...
#define NGX_LOG_TRACE             9    //调试 【trace】：最低级别

#define NGX_ERROR_LOG_PATH       "C:/ElevocTest/CalSNR.log"   //定义日志存放的路径和文件名
